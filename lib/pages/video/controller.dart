...
class VideoDetailController extends GetxController
  ...
  /// 更新画质、音质
  void updatePlayer() {
    unawaited(_updatePlayerAsync());
  }

  Future<void> _updatePlayerAsync() async {
    ...
    await _resolvePlaybackSources(firstVideo, audio: firstAudio);

    playerInit();
  }

  /// 主链路：线程撕裂者多线程加速；失败时回落到现有手动 CDN 直连。
  /// 手动 CDN 逻辑（VideoUtils.getCdnUrl）保持独立不变，仅作备用手段。
  bool _ripperActive = false;

  /// 当前是否在用多线程加速链路（false = 手动 CDN 直连回落）。
  bool get ripperActive => _ripperActive;

  Future<void> _resolvePlaybackSources(
    VideoItem video, {
    AudioItem? audio,
  }) async {
    _ripperActive = false;
    final ripper = await ThreadRipper.instance.startForDash(
      videoPlayUrls: video.playUrls,
      audioPlayUrls: audio?.playUrls ?? const <String>[],
    );
    if (ripper != null) {
      videoUrl = ripper.video;
      if (ripper.audio != null) {
        audioUrl = ripper.audio;
      } else if (audio != null) {
    ...
    videoUrl = VideoUtils.getCdnUrl(video.playUrls);
    if (audio != null) {
      audioUrl = VideoUtils.getCdnUrl(audio.playUrls, isAudio: true);
    } else {
      audioUrl = '';
  ...
  @override
  void onClose() {
    unawaited(ThreadRipper.instance.stop());
    _ripperActive = false;
    cid.close();
    if (isFileSource) {
      cacheLocalProgress();
